...
namespace sushi {
	...
	int fastgcd(int a, int b){
		if(ULKL(a == b)) return a;
    	if(ULKL(a == 0)) return b;
		if(ULKL(b == 0)) return a;
		if(ULKL(a % 2 == 0 && b % 2 == 0)) return 2 * fastgcd(a >> 1, b >> 1);
		else if(a % 2 == 0)  return fastgcd(a >> 1, b);
		else if(b % 2 == 0) return fastgcd(a, b >> 1);
	...
	uint fastgcd(uint a, uint b){
    	...
    	if(ULKL(a == 0)) return b;
		if(ULKL(b == 0)) return a;
		if(ULKL(a & 1 == 0 && b & 1 == 0)) return fastgcd(a >> 1, b >> 1) << 1;
		else if(a & 1 == 0)  return fastgcd(a >> 1, b);
		else if(b & 1 == 0) return fastgcd(a, b >> 1);
		else return fastgcd((a > b) ? (a - b) : (b - a), min(a, b));
	}
